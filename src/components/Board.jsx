...
export default function Board({ tile, onHomeClick, onCenterClick, isTouring, isDark }) {
  ...
  const renderHomeTokens = (colorClass) => (
    <div className="grid grid-cols-2 gap-2 md:gap-3 p-2 md:p-4 bg-white rounded-2xl md:rounded-3xl shadow-[inset_0_2px_6px_rgba(0,0,0,0.2)] w-2/3 h-2/3 relative z-10">
      {[...Array(4)].map((_, i) => (
  ...
  return (
    <div className="relative flex items-center justify-center z-10 w-full h-full">
      <div 
        className={`
          relative bg-white shadow-[0_10px_30px_rgba(0,0,0,0.5)] border-[3px] md:border-[6px] rounded-lg overflow-hidden select-none transition-colors duration-300
          ${isDark ? 'border-slate-800' : 'border-slate-500'}
          
          /* --- RESPONSIVE SIZING FIX --- */
          aspect-square
          w-[min(92vw,50vh)]        /* Mobile: Compact to fit dice bar */
          md:w-[min(95vw,80vh)]     /* Desktop: HUGE (Restored) */
        `}
        style={{
          display: 'grid',
      ...
      >
            ...
            <div key={i} className={`${cellClass} flex items-center justify-center relative shadow-[inset_0_0_2px_rgba(0,0,0,0.05)]`}>
               ...
               {arrowRot !== null && (
                <div className="absolute inset-0 flex items-center justify-center text-slate-400/50" style={{ transform: `rotate(${arrowRot}deg)` }}>
                    <ArrowRight size={14} className="md:w-6 md:h-6" strokeWidth={4} />
                </div>
              )}
            </div>
          );
        ...
        >
           ...
           />
           {/* TOKEN 3D */}
           <div className="w-[85%] h-[85%] bg-gradient-to-br from-yellow-400 to-orange-600 rounded-full shadow-[0_0_15px_rgba(251,146,60,0.8),0_4px_0_#b45309] border-[2px] border-white/80 relative z-10 flex items-center justify-center ring-1 md:ring-2 ring-orange-400/50">
...
const HomeBox = ({ color, label, onClick, renderTokens }) => (
  <div onClick={onClick} className={`col-span-6 row-span-6 ${color} border border-black/20 flex flex-col items-center justify-center relative cursor-pointer hover:brightness-105 active:scale-[0.99] transition-all group overflow-hidden`}>
    {/* Scaled text: Big on Desktop, Small on Mobile */}
    <span className="absolute text-[1.5rem] md:text-[3rem] font-black text-black/10 group-hover:text-black/20 transition select-none rotate-[-15deg]">{label}</span>
    {renderTokens()}
    <span className="mt-1 md:mt-2 text-[8px] md:text-xs font-bold text-black/70 bg-white/40 px-2 md:px-3 py-0.5 rounded-full backdrop-blur-sm z-10 shadow-sm border border-white/20">{label}</span>
  </div>
);
...
const CenterBox = ({ onClick }) => (
  <div onClick={onClick} className="col-span-3 row-span-3 bg-white relative border border-black/20 cursor-pointer hover:scale-105 transition z-10 shadow-lg group">
      <div className="absolute inset-0 border-[25px] md:border-[55px] border-solid border-transparent border-t-blue-500 border-r-red-500 border-b-green-500 border-l-yellow-400"></div>
      <div className="absolute inset-0 flex flex-col items-center justify-center z-20">
        <span className="text-[6px] md:text-[10px] font-black text-white drop-shadow-md tracking-widest bg-black/20 px-1 rounded mb-0.5">CONNECT</span>
      </div>
  </div>
);